...
const SUBDOMAIN_MAP: Record<string, string> = {
  ...
  "8003": DOMAIN ? `https://cs-scanner.${DOMAIN}` : "",
...
const services: ServiceInfo[] = [
  {
    ...
    longDescription:
      "PostgreSQL est le coeur de stockage de CyberSensei. Il heberge deux bases de donnees isolees : une pour l'Agent (on-premise) et une pour la Platform (SaaS). Chaque module dispose de son propre schema pour garantir l'isolation des donnees. Les migrations sont gerees automatiquement par Hibernate (Agent) et TypeORM (Platform API).",
    features: [
      "Multi-database : cybersensei_db, cybersensei_central",
  ...
  },
  {
    ...
    longDescription:
      "MongoDB est utilise exclusivement par la Platform API (SaaS). Il sert principalement de stockage GridFS pour les packages de mises a jour ZIP distribues aux instances Agent on-premise. Les metadonnees des mises a jour (version, changelog, checksum) sont stockees dans PostgreSQL, tandis que les fichiers binaires volumineux sont dans MongoDB.",
    features: [
      "GridFS pour les fichiers binaires (packages ZIP)",
      "Utilise uniquement par la Platform API",
  ...
  },
  {
    ...
    longDescription:
      "L'Agent API est le moteur principal de la plateforme de formation on-premise. Il gere l'authentification JWT, les exercices de sensibilisation (QCM, vrai/faux, simulations), les campagnes de phishing, le systeme de progression avec badges et XP, le coaching IA personnalise, et le glossaire de cybersecurite. Il communique avec le service AI pour l'analyse des prompts.",
    features: [
    ...
    usage: [
      "Connectez-vous avec vos identifiants sur l'Agent Dashboard",
  ...
  },
  {
    ...
    profile: "default, full",
    description:
      "Plateforme SaaS multi-tenant gerant les tenants, les licences, la distribution des mises a jour, le monitoring M365, le DLP, l'Infra-Scan et les rapports.",
    longDescription:
      "La Platform API est le cerveau de la plateforme SaaS. Elle gere le multi-tenancy (chaque client est un tenant isole), la creation et validation des licences, la distribution des mises a jour aux instances Agent, la collecte de telemetrie, l'audit de securite Microsoft 365 via OAuth, et la generation de rapports avec analyse IA (Claude). Elle integre desormais le module DLP (remplacant l'ancien backend Java AI Security), le module Infra-Scan et le module Reports (generation de rapports PDF via Claude, remplacant l'ancien service Python standalone). Elle utilise PostgreSQL pour les donnees structurees et MongoDB/GridFS pour les fichiers binaires.",
    features: [
      ...
      "Collecte de telemetrie depuis les instances Agent",
      "Metriques globales et tendances d'utilisation",
      "Scan de securite Microsoft 365 (10 categories)",
      "Module DLP integre : gouvernance IA, alertes, conformite RGPD (remplace l'ancien backend Java)",
      "Module Infra-Scan : orchestration des scans de vulnerabilites",
      "Module Reports : generation de rapports PDF avec resume IA (Claude), remplace l'ancien service Python standalone",
    ...
    usage: [
      "Connectez-vous au Platform Dashboard en tant qu'admin",
      "Creez des tenants pour vos clients",
      "Generez et attribuez des licences",
      "Uploadez les mises a jour de contenu",
      "Surveillez la sante des instances Agent via la telemetrie",
      "Consultez les alertes DLP et les statistiques d'utilisation IA",
      "Generez des rapports de securite via le module Reports integre",
    ],
  },
  {
    id: "platform-dashboard",
    name: "Platform Dashboard (Admin SaaS)",
    shortName: "Dashboard Platform",
    ...
    profile: "default, full",
    description:
      "Interface d'administration SaaS pour gerer les tenants, les licences, les mises a jour, le DLP, l'Infra-Scan et les rapports.",
    longDescription:
      "Le Platform Dashboard est l'interface d'administration de la plateforme SaaS. Il permet aux super-administrateurs de gerer l'ensemble des clients (tenants), de surveiller la sante de chaque instance, de distribuer les mises a jour, et de gerer les comptes administrateurs. Il integre desormais les vues Infra-Scan, DLP et Reports. Il offre une vue globale de la plateforme avec des metriques en temps reel.",
    features: [
      ...
      "Vue Infra-Scan : resultats des scans de vulnerabilites par tenant",
      "Vue DLP : alertes, statistiques d'utilisation IA, conformite RGPD",
      "Vue Reports : generation et consultation des rapports de securite",
    ],
    usage: [
  ...
  {
    id: "ai-service",
    name: "AI Service (On-Premise)",
    shortName: "AI Service",
    webUrl: buildUrl("8000", "/health"),
    icon: <Brain className="w-6 h-6" />,
    color: "#FF6F00",
    port: "8000",
    tech: "Python + FastAPI + Mistral 7B + Presidio + LLM Guard",
    techIcon: "🐍",
    profile: "ai, full",
    description:
      "Service IA unifie (DLP + analyse) deploye on-premise, combinant Presidio, LLM Guard et Mistral 7B pour la detection de donnees sensibles.",
    longDescription:
      "Le AI Service est le coeur de l'analyse DLP (Data Loss Prevention) de CyberSensei, deploye on-premise chez le client. Il resulte de la fusion des anciens services ai-dlp et ai en une instance unique. Il utilise une architecture a deux couches : la Couche 1 (rapide, 5-20ms) combine Presidio (NER) + LLM Guard pour detecter les patterns connus (emails, telephones, numeros de secu, IBAN, SIREN/SIRET, etc.). La Couche 2 (semantique, ~500ms) utilise Mistral 7B localement pour detecter les donnees sensibles exprimees en langage naturel. Il integre aussi la detection RGPD Article 9 (donnees de sante, opinions politiques, etc.) et la validation via les API gouvernementales francaises.",
    features: [
    ...
    usage: [
      "Ce service est appele automatiquement par la Platform API (module DLP) et l'extension Chrome",
  ...
  },
  {
    ...
    techIcon: "🔍",
    webUrl: buildUrl("8003", "/docs"),
    profile: "default, full",
    description:
      "API REST FastAPI qui orchestre 6 modules de scan en parallele pour analyser la surface d'attaque d'un domaine.",
    longDescription:
      "Le scanner est desormais expose comme une API REST FastAPI sur le port 8003 (et non plus en CLI uniquement). Il lance simultanement nmap (ports), nuclei (CVE), testssl.sh (TLS/SSL + SPF/DKIM/DMARC), dnstwist (typosquatting), HIBP (breaches emails) et AbuseIPDB (reputation IP). Chaque module est autonome : si un outil est absent ou echoue, les autres continuent. Le score_engine calcule un score 0-100 avec une grille de penalites detaillee. Les scans sont declenches via l'API REST ou par les schedulers de la Platform API.",
    features: [
    ...
    endpoints: [
      { method: "POST", path: "/api/scan", desc: 'Lance un scan complet synchrone {domain, emails[]}' },
      { method: "POST", path: "/api/scan/async", desc: 'Lance un scan asynchrone, retourne un scan_id' },
      { method: "GET", path: "/api/scan/{scan_id}", desc: 'Recupere le resultat d\'un scan asynchrone' },
      { method: "GET", path: "/health", desc: 'Etat de sante du service' },
    ],
    usage: [
      "curl -X POST http://localhost:8003/api/scan -H 'Content-Type: application/json' -d '{\"domain\": \"example.com\", \"emails\": [\"admin@example.com\"]}'",
      "curl -X POST http://localhost:8003/api/scan/async -d '{\"domain\": \"example.com\"}'",
      "curl http://localhost:8003/api/scan/{scan_id}",
    ],
  },
  {
    id: "compliance-nis2",
    ...
    color: "#10B981",
    port: "3006",
    tech: "NestJS + TypeORM (integre a la Platform API)",
    techIcon: "📋",
    profile: "central, full",
    description:
      "Module NestJS integre a la Platform API offrant un questionnaire NIS2 complet avec scoring, plan d'action et rapport.",
    longDescription:
...
const credentials = [
  {
    ...
    note: "Interface de gestion Agent on-premise",
  },
  {
    service: "Platform Dashboard (Admin SaaS)",
    url: buildUrl("5173"),
  ...
  },
  {
    ...
    note: "Bases : cybersensei_db, cybersensei_central",
  },
  {
...
const dockerProfiles = [
  {
    ...
    services: ["PostgreSQL", "Platform API", "Platform Dashboard", "Agent API", "Agent Dashboard", "Scanner"],
    description: "Services principaux (core)",
    color: "#22c55e",
  },
  {
    name: "tools",
    ...
    services: ["PgAdmin"],
    description: "Outils d'administration (PgAdmin)",
    color: "#3b82f6",
  },
  {
    name: "ai",
    command: "docker compose --profile ai up -d",
    services: ["AI Service (Mistral + Presidio + LLM Guard)"],
    description: "Service IA on-premise",
    color: "#f97316",
  },
  {
    name: "monitoring",
    ...
    description: "Stack de monitoring",
    color: "#8b5cf6",
  },
  {
    name: "teams",
  ...
  const stacks = [
    {
      ...
      color: "#22c55e",
      items: [
        { name: "Agent Dashboard", port: "3005", icon: "⚛️" },
        { name: "Agent API", port: "8080", icon: "☕" },
      ],
    },
    {
      name: "Platform SaaS",
      color: "#8b5cf6",
      items: [
        { name: "Platform Dashboard", port: "5173", icon: "⚛️" },
        { name: "M365 Dashboard", port: "5174", icon: "⚛️" },
        { name: "Platform API", port: "3006", icon: "🔴" },
        { name: "Scanner", port: "8003", icon: "🔍" },
      ],
    },
    {
      name: "AI Service",
...
function DataFlowDiagram() {
  ...
  const flows = [
    ...
    },
    {
      ...
      color: "#6366f1",
      detail: "Les quiz quotidiens et la progression sont geres par l'Agent API via le bot Teams",
    },
    {
      from: "Agent Instance",
      to: "Platform API",
      ...
      color: "#8b5cf6",
      detail: "Les instances on-premise envoient leur telemetrie et recuperent les mises a jour depuis la Platform API",
    },
    {
      from: "Platform API",
              ...
              {[
                {
                  ...
                  color: "#22c55e",
                  steps: [
                    "Lancez : docker compose up -d (services Agent inclus par defaut)",
                ...
                {
                  title: "2. Configurer l'AI Service (DLP)",
                  icon: <Brain className="w-5 h-5" />,
                  color: "#f97316",
                  steps: [
                    "Lancez : docker compose --profile ai up -d",
                    "Attendez le telechargement de Mistral (~4 Go)",
                    `Verifiez la sante : ${buildUrl("8000", "/health")}`,
                ...
                },
                {
                  ...
                  color: "#8b5cf6",
                  steps: [
                    "Services Platform inclus par defaut dans docker compose up -d",
          ...
          >
            ...
            <div className="space-y-2">
              {[
                ...
                { cmd: "docker compose -f docker/docker-compose.platform.yml restart platform-api", desc: "Redemarrer un service specifique" },
